...
st.set_page_config(page_title="Amazon Q and Bedrock Knowledgebase Custom UI")
st.title("Amazon Q and Bedrock Knowledgebase UI")
...
if "token" not in st.session_state:
    ...
    user_email = utils.decode_jwt(token["id_token"])["email"]
    ...
    # If the last message is from the user, generate responses from both Q and Bedrock
    if st.session_state.messages[-1]["role"] != "assistant":
        with st.chat_message("assistant"):
            with st.spinner("Thinking..."):
                placeholder = st.empty()
                
                # Amazon Q ChatSync Response
                q_response = utils.get_queue_chain(prompt, st.session_state["conversationId"],
                                                   st.session_state["parentMessageId"],
                                                   st.session_state["idc_jwt_token"]["idToken"])

                # Amazon Bedrock Knowledgebase Response
                bedrock_response = utils.get_bedrock_kb_response(prompt)
                
                full_response = f"**Amazon Q Response**:\n{q_response['answer']}\n\n"
                full_response += f"**Bedrock Response**:\n{bedrock_response['answer']}\n\n"
                
                if "references" in bedrock_response:
                    full_response += f"---\nBedrock Sources:\n{bedrock_response['references']}"
